...
adjacent_matrix <- function(dist_matrix, dist_thres){
...
##------------------------------------------------------------------------------
##                            FUNCIÓN LCC
LCC_real_network <- function(archivo, nrep){
  
  #Definimos los tamaños con los que trabajar (los mismos en todos los casos)
  tamaño_muestra <- c(100, 500, 1000, 1500, 2000)
  thres_vector <- seq(from = 5, to =300, by = 5)
  
  #Definimos la matriz en la que almacenar los valores
  LCC_tamaños <- matrix(0, 
                        nrow = length(thres_vector), 
                        ncol = length(tamaño_muestra))
  colnames(LCC_tamaños) <- c("100", "500", "1000", "1500", "2000")
  
  #Almacenamos en la matriz cada valor de LCC para cada tamaño y threshold
  for (i in 1:length(tamaño_muestra)){
    prom_val <- matrix(0, nrow = length(thres_vector), ncol = nrep)
    dist_matrix <- distance_matrix(archivo, tamaño_muestra[i])
    for (k in 1:nrep){
...
##------------------------------------------------------------------------------
##                            FUNCIÓN MODULARIDAD
Mod_real_network <- function(archivo.csv, nrep){
  ...
  for (i in 1:length(tamaño_muestra)){
    prom_val <- matrix(0, nrow = length(thres_vector), ncol = nrep)
    dist_matrix <- distance_matrix(archivo.csv, tamaño_muestra[i])
    for (k in 1:nrep){
      for (j in 1:length(thres_vector)){
...
CCoef_real_network <- function(archivo.csv, nrep){
...
##------------------------------------------------------------------------------
##                             FUNCIÓN DIÁMETRO
Diameter_real_network <- function(archivo.csv, nrep){
...
EdgeDensity_real_network <- function(archivo.csv, nrep){
